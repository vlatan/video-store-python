...
CMD sleep 5 && \
    ...
    --access-logformat '{"remote_address": "%({cf-connecting-ip}i)s", "user_name": "%(u)s", "date": "%(t)s", "status": "%(r)s", "response_code": "%(s)s", "response length": "%(b)s", "referrer": "%(f)s", "user agent": "%(a)s"}' \
    run:app